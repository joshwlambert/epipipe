---
title: "Design Principles for {epipipe}"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Design Principles for {epipipe}}
...
This vignette outlines the design decisions that have been taken during the development of the `{epipipe}` R package, and provides some of the reasoning, and possible pros and cons of each decision.
...
* The `epi_` prefix is attached to classes in {epipipe} in order to avoid conflicts with other classes used in other packages. This is also to prevent confusion two classes with the same name may have different data/attributes/methods depending on the context.
